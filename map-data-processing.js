...
async function loadGeoJson() {
    try {
      ...
      const eaResponse = await fetch('ea_data.geojson');
      if (eaResponse.ok) {
        geoJsonData = await eaResponse.json();
      } else {
        // Fallback to embedded data
        geoJsonData = EMBEDDED_EA_DATA || window.ea_data;
      }
      ...
      console.log('Loaded EA data:', geoJsonData.features.length, 'features from', eaResponse ? 'ea_data.geojson' : 'embedded');
  ...
  async function loadNcrBarangays() {
    try {
       // Load external ea_data.geojson first
      const ncrResponse = await fetch('ncr_bgy.geojson');
      if (ncrResponse.ok) {
        geoJsonData = await ncrResponse.json();
      } else {
        // Fallback to embedded data
        geoJsonData = EMBEDDED_NCR_DATA || window.ncr_bgy;
      }
